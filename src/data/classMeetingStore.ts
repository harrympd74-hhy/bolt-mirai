...
export type ClassMeeting = { id: string; number: number; title: string; className: string; startsAt: string; endsAt: string; accentColor: "teal" | "blue" | "yellow" | "cream" | "gray"; isActive: boolean; status: MeetingStatus };
...
let meetings: ClassMeeting[] = dates.map((date, index) => ({ id: `meeting-${index + 1}`, number: index + 1, title: titles[index], className: "VII-A", startsAt: localDate(date, "07:30"), endsAt: localDate(date, "09:00"), accentColor: index === 0 ? "teal" : index === 1 ? "blue" : index % 3 === 0 ? "yellow" : "cream", isActive: index === 1, status: index === 1 ? "published" : "draft" }));
